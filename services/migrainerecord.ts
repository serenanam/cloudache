...
import { markMigraineDay } from "@/services/dailyLog";
...
export async function saveMigraineRecord(userId: string, startDate: Date, intensity: number) {
  try {
    ...
    const yesterdayWeather = await getWeather(coords.latitude, coords.longitude, yesterday);

    const tempDiff24h = startTemperature - yesterdayWeather.temperature;
    const pressureDiff24h = startPressure - yesterdayWeather.pressure;
    ...
    // Mark today's daily log as a migraine day
    await markMigraineDay(userId, intensity);
